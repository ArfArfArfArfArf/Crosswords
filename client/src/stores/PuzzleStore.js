...
class PuzzleStore {
  clearExpiredData() {
    Object.entries(ls).forEach(([key, val]) => {
      ...
      if (new Date(parseInt(date[0]), parseInt(date[1]) - 1, parseInt(date[2])) < expDate) {
	delete ls[key];
      }
    });    
  }
  ...
  storePuzzle(puzzleName, puzzleYear, puzzleMonth, puzzleDay, puzzleData) {
    ls.set(`${puzzleName}-${puzzleYear}:${puzzleMonth}:${puzzleDay}`, JSON.stringify(puzzleData));
  }

  getPuzzle(puzzleName, puzzleYear, puzzleMonth, puzzleDay) {
    return JSON.parse(ls.get(`${puzzleName}-${puzzleYear}:${puzzleMonth}:${puzzleDay}`));
  }
}
